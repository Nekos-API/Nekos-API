import { Callout } from 'nextra-theme-docs';

import { APIRequest } from '../../../../components/apirequest';

# Image resource

## Search for an image

export let code1 = `
{
    "links": {
        "first": "https://api.nekosapi.com/v2/images?filter%5BverificationStatus%5D=verified",
        "last": "https://api.nekosapi.com/v2/images?filter%5BverificationStatus%5D=verified&page%5Blimit%5D=10",
        "next": null,
        "prev": null
    },
    "data": [
        {
            "type": "image",
            "id": "001a0c73-b9c6-498b-8b81-69fc3e527185",
            "attributes": {
                "file": "https://cdn.nekosapi.com/uploads/images/ZQFIROLL75DLFCZHGE5BGH7IZM.webp",
                "title": "Lumine and Paimon at the beach",
                "colors": {
                    "primary": null,
                    "dominant": null
                },
                "source": {
                    "name": null,
                    "url": null
                },
                "dimens": {
                    "height": 0,
                    "width": 0,
                    "aspectRatio": null
                },
                "isOriginal": false,
                "verificationStatus": "verified",
                "ageRating": "questionable",
                "timestamps": {
                    "created": "2023-02-13T06:43:01.214358Z",
                    "updated": "2023-02-13T19:24:49.493057Z"
                }
            },
            "relationships": {
                "uploader": {
                    "links": {
                        "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/uploader",
                        "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/uploader"
                    },
                    "data": {
                        "type": "user",
                        "id": "8fad93cd-4a9c-4e71-b21f-6eb4efebb3db"
                    }
                },
                "artist": {
                    "links": {
                        "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/artist",
                        "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/artist"
                    },
                    "data": null
                },
                "categories": {
                    "meta": {
                        "count": 0
                    },
                    "data": [],
                    "links": {
                        "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/categories",
                        "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/categories"
                    }
                },
                "characters": {
                    "meta": {
                        "count": 0
                    },
                    "data": [],
                    "links": {
                        "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/characters",
                        "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/characters"
                    }
                },
                "likedBy": {
                    "meta": {
                        "count": 0
                    },
                    "data": [],
                    "links": {
                        "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/liked-by",
                        "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/liked-by"
                    }
                }
            },
            "links": {
                "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185"
            }
        }
    ],
    "meta": {
        "pagination": {
            "count": 1,
            "limit": 10,
            "offset": 0
        }
    }
}
`.trim();

<APIRequest
    endpoint="/images"
    title="Search for an image"
    description="Search for an image using a variety of different filters."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        }
    ]}
    parameters={[
        {
            name: "include",
            type: "List of strings",
            description: "The related resources to include in the response.\n\nPossible values: `uploader`, `artist`, `categories`, `characters`, `liked-by`"
        },
        {
            name: "filter[search]",
            type: "String",
            description: "Search for an image."
        },
        {
            name: "filter[id]",
            type: "UUID",
            description: "Filter by image ID. It is recommended to use **[`/images/:id`](#get-an-image-by-its-id)** instead of this filter."
        },
        {
            name: "filter[id.in]",
            type: "UUID",
            description: "Returns images in the ID array (coma-separated. e.g. `first-uuid,second-uuid`)."
        },
        {
            name: "filter[id.regex]",
            type: "POSIX RegEx",
            description: "Returns images with IDs matching the **POSIX RegEx** specified."
        },
        {
            name: "filter[id.iregex]",
            type: "POSIX RegEx",
            description: "Returns images with IDs matching the **POSIX RegEx** specified (case-insensitive)."
        },
        {
            name: "filter[title]",
            type: "String",
            description: "Returns images matching the exact specified value."
        },
        {
            name: "filter[title.iexact]",
            type: "String",
            description: "Returns images matching the exact specified value (case-insensitive)."
        },
        {
            name: "filter[title.contains]",
            type: "String",
            description: "Returns images where the title contains the specified value."
        },
        {
            name: "filter[title.icontains]",
            type: "String",
            description: "Returns images where the title contains the specified value (case-insensitive)."
        },
        {
            name: "filter[title.startswith]",
            type: "String",
            description: "Returns images where the title starts with the specified value."
        },
        {
            name: "filter[title.endswith]",
            type: "String",
            description: "Returns images where the title ends with the specified value."
        },
        {
            name: "filter[title.regex]",
            type: "POSIX RegEx",
            description: "Returns images where the title matches the specified **POSIX RegEx**."
        },
        {
            name: "filter[title.iregex]",
            type: "POSIX RegEx",
            description: "Returns images where the title matches the specified **POSIX RegEx** (case-insensitive)."
        },
        {
            name: "filter[ageRating]",
            type: "(Enum) String",
            description: "Returns images with the specified age rating.\n\nPossible values: `sfw`, `questionable`, `borderline`, `nsfw`"
        },
        {
            name: "filter[ageRating.iexact]",
            type: "(Enum) String",
            description: "Returns images with the specified age rating (case-insensitive).\n\nPossible values: `sfw`, `questionable`, `borderline`, `nsfw`"
        },
        {
            name: "filter[ageRating.in]",
            type: "List of (enum) string",
            description: "Returns images where the age rating is in the specified list (coma-separated. e.g. `sfw,questionable`).\n\nPossible values: `sfw`, `questionable`, `borderline`, `nsfw`"
        },
        {
            name: "filter[ageRating.isnull]",
            type: "Boolean",
            description: "Returns images with an unspecified age rating.\n\nPossible values: `true`, `false`"
        },
        {
            name: "filter[ageRating.regex]",
            type: "POSIX RegEx",
            description: "Returns images where the age rating matches the **POSIX RegEx**."
        },
        {
            name: "filter[ageRating.iregex]",
            type: "POSIX RegEx",
            description: "Returns images where the age rating matches the **POSIX RegEx** (case-insensitive)."
        },
        {
            name: "filter[height]",
            type: "Number",
            description: "Returns images with the specified height."
        },
        {
            name: "filter[height.lt]",
            type: "Number",
            description: "Returns images with a height lower than the value specified."
        },
        {
            name: "filter[height.lte]",
            type: "Number",
            description: "Returns images with a height lower than or equal to the value specified."
        },
        {
            name: "filter[height.gt]",
            type: "Number",
            description: "Returns images with a height greater than the value specified."
        },
        {
            name: "filter[height.gte]",
            type: "Number",
            description: "Returns images with a height greater than or equal to the value specified."
        },
        {
            name: "filter[width]",
            type: "Number",
            description: "Returns images with the specified width."
        },
        {
            name: "filter[width.lt]",
            type: "Number",
            description: "Returns images with a width lower than the value specified."
        },
        {
            name: "filter[width.lte]",
            type: "Number",
            description: "Returns images with a width lower than or equal to the value specified."
        },
        {
            name: "filter[width.gt]",
            type: "Number",
            description: "Returns images with a width greater than the value specified."
        },
        {
            name: "filter[width.gte]",
            type: "Number",
            description: "Returns images with a width greater than or equal to the value specified."
        },
        {
            name: "filter[aspectRatio]",
            type: "String",
            description: "Returns images with the specified aspect ratio."
        },
        {
            name: "filter[aspectRatio.startswith]",
            type: "String",
            description: "Returns images where the aspect ratio starts with the specified value."
        },
        {
            name: "filter[aspectRatio.endswith]",
            type: "String",
            description: "Returns images where the aspect ratio ends with the specified value."
        },
        {
            name: "filter[aspectRatio.regex]",
            type: "POSIX RegEx",
            description: "Returns images where the aspect ratio matches the **POSIX RegEx**."
        },
        {
            name: "filter[isOriginal]",
            type: "Boolean",
            description: "Returns original (or not if false) images.\n\nPossible values: `true`, `false`"
        },
        {
            name: "filter[isOriginal.isnull]",
            type: "Boolean",
            description: "Returns images where the `isOriginal` field is null (or not if false).\n\nPossible values: `true`, `false`"
        },
        {
            name: "filter[verificationStatus]",
            type: "(Enum) String",
            description: "Returns images with the specified verification status.\n\nPossible values: `not_reviewed`, `on_review`, `declined`, `verified`"
        },
        {
            name: "filter[verificationStatus.iexact]",
            type: "(Enum) String",
            description: "Returns images with the specified verification status (case-insensitive).\n\nPossible values: `not_reviewed`, `on_review`, `declined`, `verified`"
        },
        {
            name: "filter[verificationStatus.contains]",
            type: "String",
            description: "Returns images where the verification status contains the specified value."
        },
        {
            name: "filter[verificationStatus.icontains]",
            type: "String",
            description: "Returns images where the verification status contains the specified value (case-insensitive)."
        },
        {
            name: "filter[verificationStatus.regex]",
            type: "POSIX RegEx",
            description: "Returns images where the verification status matches the specified **POSIX RegEx**."
        },
        {
            name: "filter[verificationStatus.iregex]",
            type: "POSIX RegEx",
            description: "Returns images where the verification status matches the specified **POSIX RegEx** (case-insensitive)."
        },
        {
            name: "filter[sourceName]",
            type: "String",
            description: "Returns images with the specified source name."
        },
        {
            name: "filter[sourceName.iexact]",
            type: "String",
            description: "Returns images with the specified source name (case-insensitive)."
        },
        {
            name: "filter[sourceName.contains]",
            type: "String",
            description: "Returns images where the source name contains the specified value."
        },
        {
            name: "filter[sourceName.icontains]",
            type: "String",
            description: "Returns images where the source name contains the specified value (case-insensitive)."
        },
        {
            name: "filter[sourceName.startswith]",
            type: "String",
            description: "Returns images where the source name starts with the specified value."
        },
        {
            name: "filter[sourceName.endswith]",
            type: "String",
            description: "Returns images where the source name ends with the specified value."
        },
        {
            name: "filter[sourceName.isnull]",
            type: "String",
            description: "Returns images with an unspecified (or not if false) source name.\n\nPossible values: `true`, `false`"
        },
        {
            name: "filter[sourceName.regex]",
            type: "POSIX RegEx",
            description: "Returns images where the source name matches the specified **POSIX RegEx**."
        },
        {
            name: "filter[sourceName.iregex]",
            type: "POSIX RegEx",
            description: "Returns images where the source name matches the specified **POSIX RegEx** (case-insensitive)."
        },
        {
            name: "filter[sourceUrl]",
            type: "String",
            description: "Returns images with the specified source URL."
        },
        {
            name: "filter[sourceUrl.iexact]",
            type: "String",
            description: "Returns images with the specified source URL (case-insensitive)."
        },
        {
            name: "filter[sourceUrl.contains]",
            type: "String",
            description: "Returns images where the source URL contains the specified value."
        },
        {
            name: "filter[sourceUrl.icontains]",
            type: "String",
            description: "Returns images where the source URL contains the specified value (case-insensitive)."
        },
        {
            name: "filter[sourceUrl.startswith]",
            type: "String",
            description: "Returns images where the source URL starts with the specified value."
        },
        {
            name: "filter[sourceUrl.endswith]",
            type: "String",
            description: "Returns images where the source URL ends with the specified value."
        },
        {
            name: "filter[sourceUrl.isnull]",
            type: "String",
            description: "Returns images with an unspecified (or not if false) source URL.\n\nPossible values: `true`, `false`"
        },
        {
            name: "filter[sourceUrl.regex]",
            type: "POSIX RegEx",
            description: "Returns images where the source URL matches the specified **POSIX RegEx**."
        },
        {
            name: "filter[sourceUrl.iregex]",
            type: "POSIX RegEx",
            description: "Returns images where the source URL matches the specified **POSIX RegEx** (case-insensitive)."
        },
    ]}
    responses={[
        {
            code: 200,
            name: "Success",
            description: "The request was completed successfully.",
            example: {
                code: code1,
                language: "JSON",
                headers: [
                    "application/vnd.api+json",
                    `${code1.split("\n").length} lines`,
                ],
            }
        }
    ]} />

## Get an image by its ID

export let code2 = `
{
    "data": {
        "type": "image",
        "id": "001a0c73-b9c6-498b-8b81-69fc3e527185",
        "attributes": {
            "file": "https://cdn.nekosapi.com/uploads/images/ZQFIROLL75DLFCZHGE5BGH7IZM.webp",
            "title": "Lumine and Paimon at the beach",
            "colors": {
                "primary": "#ffffff",
                "dominant": "#ffffff"
            },
            "source": {
                "name": "Pixiv",
                "url": "https://pixiv.net/some-url"
            },
            "dimens": {
                "height": 0,
                "width": 0,
                "aspectRatio": "3:4"
            },
            "isOriginal": false,
            "verificationStatus": "verified",
            "ageRating": "questionable",
            "timestamps": {
                "created": "2023-02-13T06:43:01.214358Z",
                "updated": "2023-02-13T19:24:49.493057Z"
            }
        },
        "relationships": {
            "uploader": {
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/uploader",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/uploader"
                },
                "data": {
                    "type": "user",
                    "id": "8fad93cd-4a9c-4e71-b21f-6eb4efebb3db"
                }
            },
            "artist": {
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/artist",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/artist"
                },
                "data": null
            },
            "categories": {
                "meta": {
                    "count": 0
                },
                "data": [],
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/categories",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/categories"
                }
            },
            "characters": {
                "meta": {
                    "count": 0
                },
                "data": [],
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/characters",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/characters"
                }
            },
            "likedBy": {
                "meta": {
                    "count": 0
                },
                "data": [],
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/liked-by",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/liked-by"
                }
            }
        },
        "links": {
            "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185"
        }
    }
}
`.trim();

<APIRequest
    endpoint="/images/:id"
    title="Get an image by its ID"
    description="Get an image's details by its ID."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        }
    ]}
    parameters={[
        {
            name: "id",
            type: "UUID v4",
            description: "The image's ID.",
            location: "path",
            required: true
        },
        {
            name: "include",
            type: "List of strings",
            description: "The related resources to include in the response.\nPossible values: uploader,artist,categories,characters,liked-by"
        }
    ]}
    responses={[
        {
            code: 200,
            name: "Success",
            description: "The request was completed successfully.",
            example: {
                code: code2,
                language: "JSON",
                headers: [
                    "application/vnd.api+json",
                    `${code2.split("\n").length} lines`,
                ],
            }
        }
    ]} />

## Upload an image

<Callout type="warning">
    This flow is missing an endpoint. You cannot upload images yet :/
</Callout>

export const code3 = `
{
    "data": {
        "type": "image",
        "attributes": {
            "file": null,
            "title": "Lumine and Paimon at the beach",
            "colors": {
                "primary": null,
                "dominant": null
            },
            "source": {
                "name": null,
                "url": null
            },
            "dimens": {
                "height": 0,
                "width": 0,
                "aspectRatio": null
            },
            "isOriginal": false,
            "verificationStatus": "not_reviewed",
            "ageRating": null,
            "timestamps": {
                "created": "2023-02-13T06:43:01.214358Z",
                "updated": "2023-02-13T19:24:49.493057Z"
            }
        },
        "relationships": {
            "uploader": {
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/uploader",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/uploader"
                },
                "data": {
                    "type": "user",
                    "id": "8fad93cd-4a9c-4e71-b21f-6eb4efebb3db"
                }
            },
            "artist": {
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/artist",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/artist"
                },
                "data": null
            },
            "categories": {
                "meta": {
                    "count": 0
                },
                "data": [],
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/categories",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/categories"
                }
            },
            "characters": {
                "meta": {
                    "count": 0
                },
                "data": [],
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/characters",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/characters"
                }
            },
            "likedBy": {
                "meta": {
                    "count": 0
                },
                "data": [],
                "links": {
                    "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/liked-by",
                    "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/liked-by"
                }
            }
        },
        "links": {
            "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185"
        }
    },
}
`.trim()

<APIRequest
    method="POST"
    endpoint="/images"
    title="Create an image"
    description="Create an image (file upload is done with /images/:id/file)."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Content-Type",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Authorization",
            description: "Bearer &lt;access-token&gt;",
            required: true
        }
    ]}
    parameters={[
        {
            name: "type",
            type: "Literal 'image'",
            description: "The resource type. Must be set to '`image`'.",
            required: true
        },
        {
            name: "attributes.title",
            type: "String",
            description: "The image's title (e.g `Lumine and Paimon at the beach.`).",
            required: true
        },
        {
            name: "attributes.source.name",
            type: "String",
            description: "The image's source name (e.g. `Twitter` or `Pixiv`).",
            required: true
        },
        {
            name: "attributes.source.url",
            type: "URL",
            description: "The URL of the original post (e.g. `https://twitter.com/987498237/status/8898723958`).",
            required: true
        },
        {
            name: "attributes.ageRating",
            type: "(Enum) String",
            description: "The suggested age rating.\n\nPossible values: `sfw`, `questionable`, `borderline`, `nsfw`",
            required: true
        },
        {
            name: "relationships.artist.data.type",
            type: "Literal 'artist'",
            description: "The image's artist resource type. If `relationships.artist` is set, this must be set to '`artist`'."
        },
        {
            name: "relationships.artist.data.id",
            type: "UUID",
            description: "The image's artist ID."
        },
        {
            name: "relationships.categories.data[]",
            type: "List of resources",
            description: "A list of suggested image categories.",
            required: true
        },
        {
            name: "relationships.categories.data[].type",
            type: "Literal 'category'",
            description: "The image's category resource type. This must be set to '`category`'.",
            required: true
        },
        {
            name: "relationships.categories.data[].id",
            type: "UUID",
            description: "The ID of the suggested category.",
            required: true
        },
        {
            name: "relationships.characters.data[]",
            type: "List of resources",
            description: "A list of the characters that appear in the image."
        },
        {
            name: "relationships.characters.data[].type",
            type: "Literal 'character'",
            description: "The image's character resource type. If `relationships.characters` is set, this must be set to '`character`'.",
        },
        {
            name: "relationships.characters.data[].id",
            type: "UUID",
            description: "The image's suggested character ID.",
        },
    ]}
    responses={[
        {
            code: 200,
            name: "Success",
            description: "The request was completed successfully.",
            example: {
                code: code3,
                language: "JSON",
                headers: [
                    "application/vnd.api+json",
                    `${code3.split("\n").length} lines`,
                ],
            }
        }
    ]} />


## Edit an image

<Callout type="warning">
    Since all data uploaded to the API is manually verified, once you edit an image's data it's status will become `not_verified` and will not be visible to the public until it is re-verified.
</Callout>

export const code4 = `
{
    "type": "image",
    "id": "001a0c73-b9c6-498b-8b81-69fc3e527185",
    "attributes": {
        "file": null,
        "title": "Lumine and Paimon at the beach",
        "colors": {
            "primary": null,
            "dominant": null
        },
        "source": {
            "name": null,
            "url": null
        },
        "dimens": {
            "height": 0,
            "width": 0,
            "aspectRatio": null
        },
        "isOriginal": false,
        "verificationStatus": "not_reviewed",
        "ageRating": null,
        "timestamps": {
            "created": "2023-02-13T06:43:01.214358Z",
            "updated": "2023-02-13T19:24:49.493057Z"
        }
    },
    "relationships": {
        "uploader": {
            "links": {
                "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/uploader",
                "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/uploader"
            },
            "data": {
                "type": "user",
                "id": "8fad93cd-4a9c-4e71-b21f-6eb4efebb3db"
            }
        },
        "artist": {
            "links": {
                "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/artist",
                "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/artist"
            },
            "data": null
        },
        "categories": {
            "meta": {
                "count": 0
            },
            "data": [],
            "links": {
                "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/categories",
                "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/categories"
            }
        },
        "characters": {
            "meta": {
                "count": 0
            },
            "data": [],
            "links": {
                "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/characters",
                "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/characters"
            }
        },
        "likedBy": {
            "meta": {
                "count": 0
            },
            "data": [],
            "links": {
                "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/relationships/liked-by",
                "related": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185/liked-by"
            }
        }
    },
    "links": {
        "self": "https://api.nekosapi.com/v2/images/001a0c73-b9c6-498b-8b81-69fc3e527185"
    }
}
`.trim()

<APIRequest
    method="PATCH"
    endpoint="/images/:id"
    title="Update an image"
    description="Update an image's details."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Content-Type",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Authorization",
            description: "Bearer &lt;access-token&gt;",
            required: true
        }
    ]}
    parameters={[
        {
            name: "id",
            type: "UUID",
            description: "The image's ID.",
            required: true,
            location: "path"
        },
        {
            name: "type",
            type: "Literal 'image'",
            description: "The resource type. Must be set to '`image`'.",
            required: true
        },
        {
            name: "attributes.title",
            type: "String",
            description: "The image's title (e.g `Lumine and Paimon at the beach.`)."
        },
        {
            name: "attributes.source.name",
            type: "String",
            description: "The image's source name (e.g. `Twitter` or `Pixiv`)."
        },
        {
            name: "attributes.source.url",
            type: "URL",
            description: "The URL of the original post (e.g. `https://twitter.com/987498237/status/8898723958`)."
        },
        {
            name: "attributes.ageRating",
            type: "(Enum) String",
            description: "The suggested age rating.\n\nPossible values: `sfw`, `questionable`, `borderline`, `nsfw`"
        },
        {
            name: "relationships.artist.data.type",
            type: "Literal 'artist'",
            description: "The image's artist resource type. If `relationships.artist` is set, this must be set to '`artist`'."
        },
        {
            name: "relationships.artist.data.id",
            type: "UUID",
            description: "The image's artist ID."
        },
        {
            name: "relationships.categories.data[]",
            type: "List of resources",
            description: "A list of suggested image categories."
        },
        {
            name: "relationships.categories.data[].type",
            type: "Literal 'category'",
            description: "The image's category resource type. This must be set to '`category`'."
        },
        {
            name: "relationships.categories.data[].id",
            type: "UUID",
            description: "The ID of the suggested category."
        },
        {
            name: "relationships.characters.data[]",
            type: "List of resources",
            description: "A list of the characters that appear in the image."
        },
        {
            name: "relationships.characters.data[].type",
            type: "Literal 'character'",
            description: "The image's character resource type. If `relationships.characters` is set, this must be set to '`character`'.",
        },
        {
            name: "relationships.characters.data[].id",
            type: "UUID",
            description: "The image's suggested character ID.",
        },
    ]}
    responses={[
        {
            code: 200,
            name: "Success",
            description: "The request was completed successfully.",
            example: {
                code: code4,
                language: "JSON",
                headers: [
                    "application/vnd.api+json",
                    `${code4.split("\n").length} lines`,
                ],
            }
        }
    ]} />


## Delete an image

<Callout type="error">
    This action cannot be undone.
</Callout>

<APIRequest
    method="DELETE"
    endpoint="/images/:id"
    title="Delete an image"
    description="Deletes an image from the API. This action cannot be undone."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Content-Type",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Authorization",
            description: "Bearer &lt;access-token&gt;",
            required: true
        }
    ]}
    parameters={[
        {
            name: "id",
            type: "UUID",
            description: "The image's ID.",
            required: true,
            location: "path"
        }
    ]}
    responses={[
        {
            code: 204,
            name: "Success",
            description: "The request was completed successfully.",
        }
    ]} />


## Like an image

<APIRequest
    method="POST"
    endpoint="/images/:id/like"
    title="Like an image"
    description="Adds an image to the user's liked images."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Content-Type",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Authorization",
            description: "Bearer &lt;access-token&gt;",
            required: true
        }
    ]}
    parameters={[
        {
            name: "id",
            type: "UUID",
            description: "The image's ID.",
            required: true,
            location: "path"
        }
    ]}
    responses={[
        {
            code: 204,
            name: "Success",
            description: "The request was completed successfully.",
        }
    ]} />


<APIRequest
    method="DELETE"
    endpoint="/images/:id/like"
    title="Unlike an image"
    description="Removes an image from the user's liked images."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Content-Type",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Authorization",
            description: "Bearer &lt;access-token&gt;",
            required: true
        }
    ]}
    parameters={[
        {
            name: "id",
            type: "UUID",
            description: "The image's ID.",
            required: true,
            location: "path"
        }
    ]}
    responses={[
        {
            code: 204,
            name: "Success",
            description: "The request was completed successfully.",
        }
    ]} />


## Save an image

<APIRequest
    method="POST"
    endpoint="/images/:id/save"
    title="Save an image"
    description="Adds an image to the user's saved images."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Content-Type",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Authorization",
            description: "Bearer &lt;access-token&gt;",
            required: true
        }
    ]}
    parameters={[
        {
            name: "id",
            type: "UUID",
            description: "The image's ID.",
            required: true,
            location: "path"
        }
    ]}
    responses={[
        {
            code: 204,
            name: "Success",
            description: "The request was completed successfully.",
        }
    ]} />


<APIRequest
    method="DELETE"
    endpoint="/images/:id/save"
    title="Remove an image from saved images"
    description="Removes an image from the user's saved images."
    headers={[
        {
            name: "Accept",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Content-Type",
            description: "application/vnd.api+json",
            required: true
        },
        {
            name: "Authorization",
            description: "Bearer &lt;access-token&gt;",
            required: true
        }
    ]}
    parameters={[
        {
            name: "id",
            type: "UUID",
            description: "The image's ID.",
            required: true,
            location: "path"
        }
    ]}
    responses={[
        {
            code: 204,
            name: "Success",
            description: "The request was completed successfully.",
        }
    ]} />