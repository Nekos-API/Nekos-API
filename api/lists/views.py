...
class ListViewSet(views.ModelViewSet):
    ...
    @permission_classes([permissions.IsAuthenticated])
    def add_images(self, request, pk, *args, **kwargs):
        """
        Add images to the list.
        """

        if request.POST.get("ids") is None:
            raise serializers.ValidationError(
                detail="The `ids` parameter is missing.",
        ...
        if not request.user.is_superuser:
            if request.user != images_list.user:
                raise serializers.ValidationError(
                    detail="You cannot edit a list that you do not own.",
                    code="cannot_edit_unowned_list",
                )

        images = Image.objects.filter(pk__in=request.POST.get("ids").split(","))

        if images.count() != len(request.POST.get("ids").split(",")):
            raise serializers.ValidationError(
                detail="One or more image IDs are invalid.",
                code="not_found",
            )

        images_list.images.add(images)

        return Response(data="", status=204)

    @permission_classes([permissions.IsAuthenticated])
    def remove_images(self, request, pk, *args, **kwargs):
        """
        ...
        images_list.images.remove(images)
