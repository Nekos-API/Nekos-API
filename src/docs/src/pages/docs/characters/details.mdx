import { Tab, Tabs } from "nextra-theme-docs";

# Get an Character by ID

This endpoint allows you to get an character by its ID.

## Endpoint Details

-   URL: `/v3/characters/{id}`
-   Method: `GET`
-   Response: `JSON`

## Parameters

| Location | Name | Type    | Description        | Required |
| -------- | ---- | ------- | ------------------ | -------- |
| Path     | `id` | Integer | The character's ID | Yes      |

## Example Request

<Tabs items={["Python 3", "JavaScript"]}>
    <Tabs.Tab>
        ```python filename="character_details.py" showLineNumbers
        import requests

        res = requests.get(f"https://api.nekosapi.com/v3/characters/{character_id}")
        res.raise_for_status()

        data = res.json()
        ```
    </Tabs.Tab>
    <Tabs.Tab>
        ```javascript filename="character_details.js" showLineNumbers
        fetch(`https://api.nekosapi.com/v3/characters/${character_id}`, {
            method: "GET",
        }).then(res => res.json())
        .then(data => {
            console.log(data);
        })
        ```
    </Tabs.Tab>

</Tabs>
